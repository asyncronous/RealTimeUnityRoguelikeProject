...
public class FastIKFabric : MonoBehaviour {
    ...
    void Init()
    {
        ...
        StartRotationRoot = transform.root.rotation;
    ...
    private void ResolveIK()
    {
        ...
        for (int i = 0; i < Bones.Length; i++)
        {
            Quaternion firstBoneRot = transform.rotation;

            if (i == Positions.Length - 1)
            {
                Bones[i].rotation = Target.rotation * Quaternion.Inverse(StartRotationTarget) * StartRotationBone[i];
                firstBoneRot = Bones[i].rotation;
            }
            else
            {
                Bones[i].rotation = Quaternion.FromToRotation(StartDirectionSucc[i], Positions[i + 1] - Positions[i]) * firstBoneRot;
            }
